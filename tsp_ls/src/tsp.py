...
def delta_cost_2opt(tour, instance, i, j):
    ...
    Note:
        apply_2opt reverses the segment tour[i..j], which:
          - removes edges: (tour[i-1], tour[i])  and  (tour[j], tour[j+1])
          - adds    edges: (tour[i-1], tour[j])  and  (tour[i], tour[j+1])
    """
    n = len(tour)
    ...
    current_cost = instance.distance_matrix[a][b] + instance.distance_matrix[c][d]
    new_cost     = instance.distance_matrix[a][c] + instance.distance_matrix[b][d]
...
def delta_cost_or_opt(tour, instance, i, insert_pos):
    ...
    Args:
        tour: List of vertex indices representing a tour
        instance: TSPInstance object with distance matrix
        i: Position of the vertex to move (0-based)
